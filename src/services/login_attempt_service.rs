...
pub fn register_attempt(
    ...
    if let Some(record) = records.get(key) {
        let remaining =
            record.last_attempt + chrono::Duration::minutes(BLOCK_DURATION_MINUTES) - now;
...
mod tests {
    ...
    fn case01_empty_store_accepts_an_attempt() {
        assert_eq!(
            register_attempt(&mut HashMap::new(), "a", now()),
            AttemptResult::Accepted
        );
    }
    ...
    fn case05_block_remains_at_fourteen_minutes_fifty_nine_seconds() {
        let mut records = five_attempts();
        assert_eq!(
            register_attempt(&mut records, "a", now() + chrono::Duration::seconds(899)),
            AttemptResult::Blocked(chrono::Duration::seconds(1))
        );
    }
    ...
    fn case06_block_expires_at_fifteen_minutes() {
        let mut records = five_attempts();
        assert_eq!(
            register_attempt(&mut records, "a", now() + chrono::Duration::minutes(15)),
            AttemptResult::Accepted
        );
    ...
    fn case07_block_reports_remaining_time() {
        let mut records = five_attempts();
        assert_eq!(
            register_attempt(&mut records, "a", now() + chrono::Duration::seconds(123)),
            AttemptResult::Blocked(chrono::Duration::seconds(777))
        );
    }
    ...
    fn case08_expired_attempts_restart_at_one() {
        ...
        assert_eq!(
            register_attempt(&mut records, "a", later),
            AttemptResult::Accepted
        );
        assert_eq!(records["a"].attempts, 1);
    }
    ...
    fn case10_keys_are_independent() {
        let mut records = five_attempts();
        assert_eq!(
            register_attempt(&mut records, "b", now()),
            AttemptResult::Accepted
        );
        assert!(matches!(
            register_attempt(&mut records, "a", now()),
    ...
    fn case12_cleanup_removes_expired_entries_above_threshold() {
        ...
        records.insert(
            ...
            AttemptRecord {
                attempts: 1,
                last_attempt: now() + chrono::Duration::seconds(1),
            },
        );
        cleanup(&mut records, now() + chrono::Duration::minutes(15));
