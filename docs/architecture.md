...
- 本番はRenderの無料枠（Web Service、Dockerランタイム、**シンガポールリージョン**）で稼働している。過去にKoyeb（無料枠のEcoインスタンス）への移行を設計・検証し、実際にデプロイして動作させた（#12・#15）が、**移行はせずRenderでの運用を継続する判断とした**
- 決め手は**本番DB（TiDB Serverless）との地理的距離によるレスポンスの差**。TiDBのクラスタは東京リージョンにあり、Renderのシンガポール（東京から約5,300km）に対して、Koyebで継続利用できたフランクフルト（同約9,000km）はDBとの往復が明確に遅く、**体感で分かるレベルの差**が出た（応答時間の実測は行っていない）。本アプリは1リクエストの処理でDBへ複数回往復するため、1往復あたりの遅延差がそのまま積み上がる
  - Koyebでは米東部（ワシントン）リージョンも試している。初回のデプロイは成功したものの3回目あたりからデプロイが完了しなくなり、継続して使えたのは実質フランクフルトのみだった
- ここから得た指針は**「アプリケーションのホスティング先は、DBのリージョンに合わせて選ぶ」**こと。無料枠の制約（スリープ／スケールtoゼロ）はKoyeb・Renderのどちらにも同様に存在し、この点は選定の決め手にならなかった
...
## 25. 管理者ログインのレート制限（`POST /auth/login`）

### 背景

`POST /auth/login` は、ユーザー名を持たない**単一パスワード認証**であり、試行回数の制限が一切なかった。リポジトリを一般公開すると、この認証設計とエンドポイント構成が既知になるため、本番URLが判明した場合にオンラインでのパスワード総当たりを妨げるものが Argon2 の計算コストのみになる。総当たりの成立を実用上不可能な水準まで下げるため、試行回数の制限を導入する。

### 方式: プロセス内メモリでの試行回数記録

- 試行回数は**プロセス内メモリ**（`AppState` が持つ `Arc<Mutex<HashMap<..>>>`）に保持する。セッションストア（`MemoryStore`）・スタンプ画像キャッシュ（23節）と同じ方針
- DB永続化は採用しない。理由は2点。(1) 失敗試行のたびに本番DB（東京リージョン）への書き込みが発生し、18節で述べたレイテンシをそのまま攻撃者に肩代わりさせる形になる。(2) カウンタが失われる契機は「15分以上アクセスが途絶えてインスタンスがスリープする」ことだが、総当たり攻撃が継続している間はアクセスが途切れないためプロセスは生き続ける。**攻撃継続中はカウンタも維持される**ため、メモリ保持で目的を達する
- 参加登録の一時状態（9節）をDB永続化した判断とは前提が異なる。あちらは状態が失われると参加者の体験が壊れる（実害がある）のに対し、こちらは失われても「攻撃が既に止んでいる」状況に限られる

### キー: `X-Forwarded-For` の最右値

- `axum::serve` を `into_make_service_with_connect_info` なしで呼んでいるため、接続元のソケットアドレスはハンドラーから取得できない。またRenderはプロキシ経由であり、ソケットアドレスが取れてもプロキシのアドレスにしかならない
- したがってクライアントIPは `X-Forwarded-For` ヘッダーから取得する。**値が複数ある場合は最も右の値を採用する**。このヘッダーは各プロキシが「自身が観測した接続元」を追記していく仕様であり、クライアントが偽装できるのは左側の値だけであるため、最右値が信頼できる唯一の値になる（左端を採用すると、攻撃者が任意のIPを詐称してブロックを回避できる）
- ヘッダーが存在しない・パースできない場合は、`unknown` という固定キーを共有バケットとして使う。Renderのプロキシは常にこのヘッダーを付与するため、本番でこの経路に入るのはヘッダーを剥がす経路を通った場合のみ。ローカル開発では常に共有バケットになるが、開発上の支障はない
- 同名ヘッダーが複数行に分かれて届く場合も、**最後の行**が最も信頼できる（プロキシは末尾に追記するため）。最後の行から値を取り出せないとき（UTF-8として解釈できない、値が空など）は、それより前の行に遡らず `unknown` にフォールバックする。前の行はクライアントが送った可能性のある値であり、そこへ遡ることは攻撃者の制御下にある値をキーに採用することを意味するため

### 判定ロジック

判定は `login_attempt_service` に純粋関数として切り出し、現在時刻を引数で受け取る（テストで時間を進められるようにするため。`std::thread::sleep` を使うテストは書かない）。

- 同一キーからの試行が**5回**に達した時点で、そのキーを**15分間**ブロックする
- ブロック中の試行は、パスワードの照合を行わずに `429 Too Many Requests` を返す（Argon2の計算とDBアクセスをスキップする。攻撃時のCPU・DB負荷を増やさないため）。`Retry-After` ヘッダーには残り秒数を**切り上げた**整数を入れる（切り下げると、その秒数で再試行しても依然ブロック中になり、クライアントを無駄に往復させるため）
- 最後の試行から15分以上経過している場合、それ以前の試行回数は無効として1回目から数え直す
- ログインに成功した時点で、そのキーの記録を削除する

#### カウントは「照合の前」に行う — 競合の排除

**ブロック判定と回数の加算は、ひとつのロック区間の中で不可分に行い、パスワード照合より前に加算を完了させる。**

判定（読み取り）と加算（書き込み）を別々のロック区間に分け、その間にパスワード照合（Argon2 + DB往復で約100ms）を挟む実装にしてはならない。同一キーから同時に多数のリクエストが到着した場合、そのすべてが「まだ0回」の状態で判定を通過し、回数制限が事実上「同時実行数の制限」に置き換わる。5回という上限が並行アクセス下で意味を失い、総当たりを妨げられなくなる。

したがって処理は次の順序になる。

1. ロックを取得し、その中で「ブロック判定 → ブロックされていなければ試行回数を1増やす」を行い、ロックを解放する
2. ブロック中であれば `429` を返す（照合は行わない）
3. パスワードを照合する
4. 成功したらそのキーの記録を削除する（回数のリセット）
5. 照合が失敗した場合も、DBエラー等で**照合の成否が判定できなかった**場合も、何もしない（手順1で加算済みのため）

この方式では加算が「失敗回数」ではなく「試行回数」を数えることになるが、正しいパスワードでの試行は手順4で記録ごと削除されるため、正規の管理者に対する挙動は変わらない（4回失敗した後に正しいパスワードでログインできる）。

**カウンタを減らす経路は「認証成功」と「時間経過」だけとし、加算を取り消す経路は設けない。** DBエラーで照合の成否が判定できなかった試行も、取り消さずに数えたままにする。一見すると「パスワードの誤りが確認できていない試行を失敗として数える」のは不当に見えるが、取り消しを許すと次の3つの問題が生じる。

- DB障害が続いている間、すべての試行が取り消されてカウンタが上限に到達せず、**レート制限が事実上停止する**。バックプレッシャーが最も必要な局面で失われる
- 取り消しは「いつ加算した分か」を区別できない。`record_success` や15分の減衰を跨いで取り消しが届くと、別の期間のカウンタを減らしてしまう
- カウンタが単調でなくなるため、並行動作を検証するテストがDBの状態に依存して結果を変える

代償として「DBエラーが5回続くと、復旧後も15分間はログインできない」状態が起こりうる。ただしDB障害中はどのみち認証自体が成立しないうえ、時間経過で自然に解消する。レート制限が無効化されることの方が重いと判断した。

#### 記録の掃除

- キーごとの記録は、期限切れ分（最終更新から15分以上経過）を掃除する。掃除しない場合、送信元IPを変えながら試行されるとマップが際限なく成長してメモリを圧迫するため
- ただし**毎リクエストで全件走査してはならない**。走査はロックを保持したまま行われるため、記録件数に比例してロック保持時間が伸び、送信元IPを変え続ける攻撃で全ログイン要求が直列化する（攻撃が自身を増幅させる形になる）。記録件数がしきい値を超えたときにのみ走査することで、コストを償却する

### 受容する残存リスク

以下は把握したうえで対策しない。いずれも対策のコストが、対象規模（1建物・1イベント・管理者1人、無料枠ホスティング）に見合わないと判断した。

- **記録件数がしきい値を超え続ける規模の分散攻撃**: しきい値超過後は再び毎リクエストで走査が走り、ロック保持時間が伸びる。ただしキーは実際の接続元IPであり、しきい値を超えるには多数の実IPを要する。その規模の攻撃者は、そもそも無料枠インスタンスをトラフィックだけで飽和させられるため、この経路を塞いでも防御にならない
- **`unknown` 共有バケットの巻き添え**: `X-Forwarded-For` を持たないリクエストは単一のキーを共有するため、その中の1つが5回失敗すると他も巻き添えでブロックされる。Renderでは全通信がプロキシを経由しヘッダーが必ず付くため、本番で該当するのはプロキシを迂回した経路のみ。ローカル開発では常に共有バケットになるが支障はない
- **`std::sync::Mutex` の poisoning**: ロック保持中にパニックが起きると、以降のログインが恒久的に500になる。臨界区間は `HashMap` 操作のみでパニック経路が無いため、発生の見込みが立たない

### 意図的に採用しなかった方式

- **全体（グローバル）での試行回数上限**: 送信元を問わず一定回数でログインを封鎖する方式は、第三者が意図的に失敗を繰り返すだけで正規の管理者を締め出せる（DoS）。管理者が1人で、イベント当日に管理画面へ入れないことの被害が大きいため採用しない
- **アカウントロック（恒久的な凍結）**: 解除手段がDB直接操作しかなく、運用が成立しない
- **CAPTCHA・多要素認証**: 対象規模（1建物・1イベント・管理者1人）に対して導入・運用コストが見合わない

### 制限の対象外

- CSRFトークン不正（`403`）は失敗回数に数えない。CSRF検証はパスワード照合より前に行われ、トークンはログイン画面から誰でも取得できるため、回数制限の指標として意味を持たない
