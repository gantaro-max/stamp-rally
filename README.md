...
<details>
<summary><b>ホスティング先は DB のリージョンに合わせて選ぶ</b></summary>

本番 DB（TiDB Serverless）は東京リージョンにある。移行を検討して実際にデプロイした Koyeb では、継続利用できたのがフランクフルト（東京から約 9,000km）だったのに対し、Render はシンガポール（同約 5,300km）を選べた。1 リクエストの処理で DB へ複数回往復する構造のため、1 往復あたりの遅延差がそのまま積み上がり、**体感で分かるレベルでフランクフルト側が遅かった**（応答時間の実測は行っていない）。無料枠の制約（スリープ / スケール to ゼロ）は両者に同様にあり選定の決め手にならず、**DB との距離**で Render を継続採用した。
（[architecture.md 18節](docs/architecture.md#18-デプロイ構成render)）
</details>
...
<details>
<summary><b>ログイン試行の制限は「照合の前」に数える</b></summary>

管理画面のログインはユーザー名を持たない単一パスワード認証のため、総当たり対策として同一送信元からの試行回数を制限している。素朴に実装すると「判定 → パスワード照合（Argon2 + DB、約 100ms）→ 失敗なら記録」という順序になりがちだが、これだと判定と記録の間の約 100ms の隙間を突かれ、**同時に大量のリクエストを送るだけで全リクエストが「まだ 0 回」の状態で判定を通過してしまう**。実際にこの実装で提出された PR を最終レビューで検出し、2 度差し戻した（後述）。正しくは、判定と加算を単一のロック区間で不可分に行い、照合より前に完了させる。
（[architecture.md 25節](docs/architecture.md#25-管理者ログインのレート制限post-authlogin)）
</details>
...
squash マージ後も元のコミット粒度が残っている PR は、**`test:`(失敗するテスト)のコミットから始めている。**
例外は初期セットアップ(#2)と、振る舞いを変えない変更(#11)の 2 件のみ。
...
前掲の「Webhook のイベント単位 spawn による順序問題」「ログイン試行の制限は『照合の前』に数える」は、このレビューで検出して差し戻した実例である。とくに後者は、指示書どおりに再実装した PR も一度は「テストケースの網羅漏れ」で再度差し戻しており、履歴からその経緯をそのまま追える。
...
```bash
cargo test      # テスト（238 件）
cargo clippy    # Lint
```

> **DB 結合テストについて**
> 238 件のうち 136 件は `#[sqlx::test]` による DB 結合テストで、テストごとに使い捨てのデータベース（`_sqlx_test_*`）を作成する。devcontainer を使う場合は `postCreateCommand` が必要な権限を自動で付与するため、`cargo test` をそのまま実行できる。devcontainer を使わず自前で DB を用意する場合は、`DATABASE_URL` のユーザーに `_sqlx_test_*` という名前のデータベースを作成・削除できる権限が必要となる。
>
