...
4. Claude: 基本設計を踏まえた実装指示書を作成（詳細設計を兼ねる）
        ↓
5. Codex: `feature/<機能名>` ブランチ上でTDDに基づいてコードを実装し、Pull Requestを作成する
        ↓
6. Claude: 複数サブエージェントによる最終レビューを実施（後述、PR上の変更内容を確認する）
        ↓
7. Claude: ドキュメントを更新（CHANGELOG・各設計書等）
        ↓
8. PRをSquash mergeで`main`にマージし、featureブランチを削除・デプロイ
```
...
| TDD遵守レビュー | 指示書のテストケースに対応するテストが存在し、Red-Green-Refactorが個別のコミットとしてgit履歴から確認できるか（実装を正当化するためだけの後付けテストになっていないか） |
...
- `main` への直接コミットは禁止。すべての実装は `feature/<機能名>` ブランチ（[AGENTS.md](AGENTS.md) 参照）とPull Requestを経由する
- 最終レビューはPR上の変更内容に対して行う
- 最終レビュー通過後、PRを **Squash merge** で `main` にマージし、featureブランチを削除する
- ドキュメント更新（本ワークフローのステップ7）は、マージ前後どちらでもよいが、マージ後の`main`の状態と設計書に矛盾が生じないようにする
