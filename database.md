...
|:--|:--|:--|
| `id` | INT(PK) | 自動採番 |
| `event_id` | INT(FK) | 所属イベント（`events` 参照） |
| `room_name` | VARCHAR | 部屋名 |
| `quest_text` | TEXT | 部屋到着時に提示するクエスト文 |
| `answer` | VARCHAR(NULL可) | 正解キーワード（カンマ区切りで複数可）。`require_answer_check = true` のイベントでのみ使用 |
| `hint_msg` | VARCHAR(NULL可) | ヒントメッセージ。`require_answer_check = true` のイベントでのみ使用 |
...
| `player_name` | VARCHAR | 個人戦: 個人名 / チーム戦: チーム名 |
| `current_room_id` | INT(FK, NULL可) | 現在案内している部屋（`rooms` 参照） |
...
| カラム | 型 | 説明 |
|:--|:--|:--|
| `player_id` | INT(FK) | プレイヤー（`players` 参照） |
| `room_id` | INT(FK) | 訪問済みの部屋（`rooms` 参照） |
...
| `mime_type` | VARCHAR | MIMEタイプ（`image/jpeg` 等） |
